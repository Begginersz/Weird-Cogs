...
class Antilink:
    ...
    can optionally block all links not just invites. Inspired from cogs by Kowlin."""

    __author__ = "Weirdo914"
    __version__ = "1"
                ...
                if self.json[message.server.id]['strict']:
                    for match in self.regex_url.finditer(message.content):
                        if self.emoji_string not in match.group(0):
                            ...
                            if (check.startswith("<:") and check.endswith(">")) or (check.startswith("<a:") and check.endswith(">")):
                                return
                            else:
                                asyncio.sleep(0.5)
